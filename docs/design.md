...
- **`email_home_address`** is **required** when `email_enabled`; it sets **`EMAIL_HOME_ADDRESS`** in Compose—the email analogue of **`SLACK_HOME_CHANNEL`** (default address for cron-style job delivery).
